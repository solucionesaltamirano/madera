...
class MenuHasItemsLivewire extends Component
{
    ...
    public function render()
    {
        ...
        $moduleLists = $routeLists->unique('modul')->pluck('modul' )->all();

        $modules = collect();
        foreach($moduleLists as $module){
            $item = new stdClass;
            $item->group = $routeLists->where('modul', $module)->pluck('group')->first();
            $item->name = $module;
            $modules->push($item);
        }

        return view('livewire.menus.menu-has-items-livewire', [
